...
func (r RealClock) NewDeadlineTimer(t time.Time) Timer {
	return &realTimer{
		timer: time.NewTimer(r.Since(t)),
	}
}
...
func (f *FakeClock) NewTimer(d time.Duration) Timer {
	...
	defer f.lock.Unlock()
	stopTime := f.time.Add(d)
	return f.newTimerLocked(stopTime)
}

func (f *FakeClock) NewDeadlineTimer(t time.Time) Timer {
	f.lock.Lock()
	defer f.lock.Unlock()
	return f.newTimerLocked(t)
}
...
func (f *FakeClock) NewTicker(d time.Duration) Ticker {
	...
	f.setTimeLocked(f.time) // try to fire this waiter
	return &fakeTicker{
		c: ch,
	}
}
...
func (f *FakeClock) newTimerLocked(stopTime time.Time) Timer {
	ch := make(chan time.Time, 1) // Don't block!
	timer := &fakeTimer{
		fakeClock: f,
...
func (*IntervalClock) NewDeadlineTimer(t time.Time) Timer {
	panic("IntervalClock doesn't implement NewDeadlineTimer")
}
